#!/usr/bin/env python3
"""
Scenario Analysis V6: TTK Combat + Trade System Integration

Philosophy:
- Economy outcomes EMERGE from weapon TTK
- Man advantage EMERGES from trade opportunities
- No hardcoded "5v4 = +10% bonus"

Key changes from V5:
- Integrated trade system for post-kill mechanics
- Trade opportunities based on distance, LOS, reaction time
- Numbers advantage emerges naturally from trade potential
"""
...
# =============================================================================
# VCT-DERIVED ROLE-BASED POSITIONING (from behavioral_patterns.json + analysis)
# =============================================================================
# VCT Data shows:
# - Duelists: avg 1600 units from site (closest), high spread
# - Initiators: avg 1800 units, medium spread
# - Sentinels: avg 1850 units, anchor more (lower spread)
# - Controllers: avg 1900 units (furthest), high variance for smoke angles

ROLE_POSITIONING = {
    'duelist': {
        'base_distance': 0.12,      # Closest to spike (normalized: ~1200 units)
        'distance_variance': 0.08,   # High variance - aggressive angles
        'tight_prob': 0.05,          # 5% tight clustering
        'medium_prob': 0.20,         # 20% medium spread
        'spread_prob': 0.75,         # 75% spread out
    },
    'initiator': {
        'base_distance': 0.15,       # Mid-range positioning
        'distance_variance': 0.08,
        'tight_prob': 0.10,
        'medium_prob': 0.35,
        'spread_prob': 0.55,
    },
    'sentinel': {
        'base_distance': 0.16,       # Anchors - mid-far from spike
        'distance_variance': 0.06,   # Lower variance (anchor positions)
        'tight_prob': 0.20,          # 20% tight - anchor tight angles
        'medium_prob': 0.35,
        'spread_prob': 0.45,
    },
    'controller': {
        'base_distance': 0.18,       # Furthest - smoke angles
        'distance_variance': 0.10,   # High variance for utility angles
        'tight_prob': 0.10,
        'medium_prob': 0.25,
        'spread_prob': 0.65,
    },
...
# VCT spread distribution (overall team): 10% tight, 25% medium, 65% spread
VCT_SPREAD_DISTRIBUTION = {
    'tight': (0.02, 0.08),      # 200-800 units (normalized)
    'medium': (0.08, 0.16),     # 800-1600 units
    'spread': (0.16, 0.30),     # 1600-3000 units
}
...
# From trade_patterns.json: 25.2% trade rate, 1.72s avg trade time
TRADE_WINDOW_MS = 1700  # Average trade time
MAX_TRADE_WINDOW_MS = 3000  # Maximum trade window

# Trade probability by time (from VCT distribution)
# 40% under 1s, 26% 1-2s, 14% 2-3s, 21% 3-5s
TRADE_PROB_UNDER_1S = 0.70
TRADE_PROB_1_TO_2S = 0.50
TRADE_PROB_2_TO_3S = 0.30
TRADE_PROB_OVER_3S = 0.15

# Distance factors (normalized map units, map is roughly 0-1)
MAX_TRADE_DISTANCE = 0.15  # Must be within 15% of map to trade (tuned)
OPTIMAL_TRADE_DISTANCE = 0.08  # Close range for optimal trades

# Position advantage (tuned parameter)
POSITION_ADVANTAGE_PCT = 0.15  # 15% of engagements have clear position advantage
...
class TradeIntegratedAnalyzer:
    """Scenario analyzer with integrated trade system."""
    ...
    async def run_all_scenarios(self, rounds_per_scenario: int = 50):
        print(f"\n{'='*70}")
        print("SCENARIO ANALYSIS V6 - TTK COMBAT + TRADE SYSTEM")
        print(f"{'='*70}")
        print("Key changes:")
        print("  - TTK-based combat (economy emerges from weapon stats)")
        print("  - Trade system integrated (man advantage emerges from trades)")
        print("  - VCT data: 25% trade rate, 1.7s avg trade time")
        print("  - No hardcoded scenario bonuses")
        ...
        for scenario in Scenario:
            ...
            trade_info = f"(trades: {result.get('avg_trades', 0):.1f})"
            print(f"{status} Attack Win: {actual:.0f}% (target: {target:.0f}%, diff: {diff:+.0f}%) {trade_info}")
    ...
    def _get_vct_post_plant_position(
        self,
        site_center: Tuple[float, float],
        agent: str
    ) -> Tuple[float, float]:
        """Generate VCT-accurate post-plant position using role-based distribution.

        VCT data shows attackers are SPREAD from the start (57-66% >2000 units apart).
        This method matches that distribution with role-specific biases.

        Args:
            site_center: Center position of the spike site
            agent: Agent name for role determination

        Returns:
            Position tuple (x, y) in normalized coordinates
        """
        role = AGENT_ROLES.get(agent, 'duelist')
        role_config = ROLE_POSITIONING.get(role, ROLE_POSITIONING['duelist'])

        # Determine spread category using role-specific probabilities
        roll = random.random()
        if roll < role_config['tight_prob']:
            spread_category = 'tight'
        elif roll < role_config['tight_prob'] + role_config['medium_prob']:
            spread_category = 'medium'
        else:
            spread_category = 'spread'

        # Get spread range for this category
        spread_min, spread_max = VCT_SPREAD_DISTRIBUTION[spread_category]

        # Apply role base distance bias
        base_dist = role_config['base_distance']
        variance = role_config['distance_variance']

        # Calculate final distance with some randomness
        distance = base_dist + random.uniform(-variance, variance)
        distance = max(spread_min, min(spread_max, distance))

        # Random angle from site center
        angle = random.uniform(0, 2 * math.pi)

        # Calculate position
        x = site_center[0] + distance * math.cos(angle)
        y = site_center[1] + distance * math.sin(angle)
    ...
    # AUTONOMOUS PLAYERS: Real player names for VCT data lookup
    C9_PLAYERS = ['OXY', 'xeppaa', 'v1c', 'Jakee', 'mitch']
    OPPONENT_PLAYERS = ['aspas', 'Sato', 'kiNgg', 'C0M', 'tex']
    ...
    def _setup_players(self, engine, map_data, config):
        ...
        for i in range(config['num_attackers']):
            ...
            player_name = self.C9_PLAYERS[i % len(self.C9_PLAYERS)]
            agent = self.AGENTS_ATTACK[i % len(self.AGENTS_ATTACK)]

            # AUTONOMOUS: Load real player profile
            profile = data_loader.get_player_profile(player_name)

            if config['spike_planted'] and config['spike_site']:
                # VCT-accurate post-plant positioning with role-based spread
                site_data = map_data['sites'][config['spike_site']]
                spawn = self._get_vct_post_plant_position(
                    site_data['center'],
                    agent
                )
            else:
            ...
            # AUTONOMOUS: Use real stats if available
            if profile:
                hs_rate = profile.headshot_rate if profile.headshot_rate > 0 else 0.25
                aggression = profile.aggression
                clutch = profile.clutch_potential
                trade_awareness = min(0.9, profile.kd_ratio * 0.5) if profile.kd_ratio else 0.6
                pref_distance = profile.avg_kill_distance / 10000.0 if profile.avg_kill_distance else 0.18
            else:
                hs_rate = 0.25
            ...
            player = SimulatedPlayer(
                ...
                preferred_engagement_distance=pref_distance,
        ...
        for i in range(config['num_defenders']):
            ...
            player_name = self.OPPONENT_PLAYERS[i % len(self.OPPONENT_PLAYERS)]
            ...
            for attacker in list(engine.players.values()):
                ...
                for defender in list(engine.players.values()):
                    ...
                    if random.random() < combat_chance:
                        ...
                        # =====================================================
                        # TRADE SYSTEM: Check if teammates can trade the killer
                        # =====================================================
                        trade_result = self._check_for_trade(
                            kill_event, engine, time_ms, config
                        )
                        if trade_result:
                            # Trade succeeded - killer dies
                            ...
                            kills.append(KillEvent(
                                ...
                                killer_side=loser.side,  # Trader is on victim's team
    ...
    def _check_for_trade(
        ...
        Returns dict with trade info if successful, None otherwise.
        """
        # Find potential traders (teammates of victim who are alive)
        potential_traders = [
        ...
        # Get killer info for weapon comparison
        ...
        # Sort by distance to killer (closest first)
        ...
        for trader in potential_traders:
            ...
            # Weapon factor: TTK advantage affects trade success
            # Classic vs Vandal: Classic has ~2x slower TTK, so much lower trade success
    ...
    def _get_weapon_trade_factor(self, trader_weapon: str, killer_weapon: str) -> float:
        """
        Get weapon factor for trade success.

        Based on TTK ratios: Classic ~2x slower than Vandal.
        If trader has worse weapon, lower chance of successful trade.
        """
        # Relative TTK effectiveness (1.0 = baseline rifle)
        weapon_effectiveness = {
            'vandal': 1.0,
            'phantom': 1.0,
            'spectre': 0.7,
            'stinger': 0.6,
            'classic': 0.4,  # Much worse for trading
            'ghost': 0.5,
            'sheriff': 0.6,
            'frenzy': 0.4,
            'shorty': 0.3,
        ...
        # Ratio determines trade success modifier
        # If trader has Classic (0.4) vs killer with Vandal (1.0), factor = 0.4
        # If equal weapons, factor = 1.0
        ratio = trader_eff / killer_eff

        # Clamp between 0.2 and 1.2
        return max(0.2, min(1.2, ratio))

    def _resolve_combat_ttk(self, player_a, player_b, config, distance: float) -> Tuple[str, str]:
        """
        ...
        # Build combat states - players STOP MOVING when they engage
        # In real Valorant, you stop to aim and shoot
        state_a = CombatState(
            health=100,
            armor=player_a.shield,
            is_moving=False,  # Players stop to shoot
        ...
        # Probabilistic position advantage (tuned parameter)
        # Not every engagement has clear position advantage
        spike_planted = config.get('spike_planted', False)
        has_clear_position = random.random() < POSITION_ADVANTAGE_PCT
    ...
    def _get_post_plant_phase(self, time_ms: int, plant_time: int) -> str:
        """Determine post-plant sub-phase for movement patterns."""
        elapsed = time_ms - plant_time
        if elapsed < 10000:
            return 'post_plant_early'  # Setup phase
        elif elapsed < 25000:
            return 'post_plant_mid'    # Hold phase
        else:
            return 'post_plant_late'   # Late pressure

    def _get_target(self, player, engine, map_data, config) -> Tuple[float, float]:
        """Get movement target for player.

        VCT data shows 99.5% of attackers are mobile post-plant:
        - Early: Move to setup positions based on role
        - Mid: Patrol between angles (60% running, 40% walking)
        - Late: Aggressive positioning / hunt
        """
        if player.side == 'attack':
            if config.get('spike_planted'):
                # VCT: 99.5% mobile post-plant, NOT static
                site_data = map_data['sites'][config['spike_site']]
                site_center = site_data['center']
                role = AGENT_ROLES.get(player.agent, 'duelist')

                # Determine post-plant phase for movement pattern
                plant_time = getattr(engine, 'spike_plant_time', config.get('start_time_ms', 0) - 5000)
                current_time = config.get('start_time_ms', 0)
                phase = self._get_post_plant_phase(current_time, plant_time)

                # Role-based movement patterns
                role_config = ROLE_POSITIONING.get(role, ROLE_POSITIONING['duelist'])

                if phase == 'post_plant_early':
                    # Setup phase: move to hold positions
                    # Duelists closer, controllers further for angles
                    target_dist = role_config['base_distance']
                    angle = random.uniform(0, 2 * math.pi)
                    return (
                        site_center[0] + target_dist * math.cos(angle) + random.uniform(-0.03, 0.03),
                        site_center[1] + target_dist * math.sin(angle) + random.uniform(-0.03, 0.03)
                    )

                elif phase == 'post_plant_mid':
                    # Patrol phase: 80% chance to move to new angle
                    if random.random() < 0.80:
                        # Patrol to a different angle
                        patrol_dist = role_config['base_distance'] + random.uniform(-0.05, 0.05)
                        new_angle = random.uniform(0, 2 * math.pi)
                        return (
                            site_center[0] + patrol_dist * math.cos(new_angle),
                            site_center[1] + patrol_dist * math.sin(new_angle)
                        )
                    else:
                        # Hold current position with micro-adjustments
                        return (
                            player.x + random.uniform(-0.02, 0.02),
                            player.y + random.uniform(-0.02, 0.02)
                        )

                else:  # post_plant_late
                    # Aggressive phase: duelists hunt, others tighten
                    if role == 'duelist':
                        # Hunt toward likely defender positions
                        hunt_dist = 0.20 + random.uniform(-0.05, 0.05)
                        angle = random.uniform(0, 2 * math.pi)
                        return (
                            player.x + hunt_dist * math.cos(angle),
                            player.y + hunt_dist * math.sin(angle)
                        )
                    else:
                        # Others tighten up around site
                        tighter_dist = role_config['base_distance'] * 0.7
                        angle = random.uniform(0, 2 * math.pi)
                        return (
                            site_center[0] + tighter_dist * math.cos(angle),
                            site_center[1] + tighter_dist * math.sin(angle)
                        )
            else:
                # Pre-plant: move toward site
    ...
    def print_results(self):
        print(f"\n{'='*70}")
        print("V6 RESULTS - TTK COMBAT + TRADE SYSTEM")
        print(f"{'='*70}")
        ...
        for category, scenarios in categories.items():
            ...
            for scenario in scenarios:
                if scenario in self.results:
                    ...
                    print(f"  {status} {scenario.value:25s} │ {bar} │ {actual:5.1f}% (target: {target:.0f}%, trades: {trades:.1f})")
...
async def main():
    analyzer = TradeIntegratedAnalyzer()
    ...
    output_file = Path(__file__).parent / 'scenario_analysis_v6_results.json'
    ...
    with open(output_file, 'w') as f:
        json.dump({
            ...
            'accuracy': accuracy,
            'version': 'v6_ttk_trade_integrated',
            'key_changes': [
                'ttk_based_combat',
                'trade_system_integrated',
                'man_advantage_from_trades',
                'no_hardcoded_bonuses',
            ],
            'parameters': {
                'trade_window_ms': TRADE_WINDOW_MS,
                'max_trade_distance': MAX_TRADE_DISTANCE,
                'trade_prob_under_1s': TRADE_PROB_UNDER_1S,
                'trade_prob_1_to_2s': TRADE_PROB_1_TO_2S,
            },
