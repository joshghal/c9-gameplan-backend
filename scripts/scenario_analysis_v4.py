#!/usr/bin/env python3
"""
Scenario Analysis V4 - Weapon Advantage Fix

Key fix: Separates WEAPON ADVANTAGE from NUMBERS ADVANTAGE
- Numbers advantage: Capped at ±10% (based on player count)
- Weapon advantage: NOT capped, 25-35% difference for eco vs full buy

Previous issues fixed:
- eco_vs_full was 53% (target 15%) - now properly penalized
- full_vs_eco was 59% (target 85%) - now properly boosted
"""
...
# Weapon tiers for advantage calculation
WEAPON_TIERS = {
    'classic': 0.3,    # Pistol
    'ghost': 0.4,      # Better pistol
    'sheriff': 0.5,    # High skill pistol
    'spectre': 0.6,    # SMG
    'bulldog': 0.7,    # Budget rifle
    'guardian': 0.75,  # Semi-auto
    'phantom': 0.9,    # Top rifle
    'vandal': 0.9,     # Top rifle
    'operator': 0.95,  # AWP
}
...
class CombatContext:
    ...
    attacker_has_crossfire: bool
    defender_has_crossfire: bool
    attacker_flashed: bool
    defender_flashed: bool
    is_retake: bool
    spike_time_remaining: int
    attacker_on_site: bool
    defender_on_site: bool
    attacker_weapon_tier: float  # NEW: weapon quality 0-1
    defender_weapon_tier: float  # NEW: weapon quality 0-1
    attacker_has_shield: bool    # NEW: armor
    defender_has_shield: bool    # NEW: armor
...
class FixedScenarioAnalyzerV4:
    """Scenario analyzer with weapon advantage fix."""
    ...
    # V3 parameters (kept)
    FLASH_ACCURACY_PENALTY = 0.70
    FLASH_DURATION_MS = 2000
    CROSSFIRE_BONUS = 0.12
    NUMBERS_ADVANTAGE_CAP = 0.10
    NUMBERS_ADVANTAGE_PER_PLAYER = 0.03

    # V5 FIX: Balanced post-plant mechanics
    DEFUSE_PRESSURE_THRESHOLD = 20000  # 20 seconds
    DEFUSE_PRESSURE_BONUS = 0.05  # Small defender bonus when defusing under pressure
    RETAKE_UTILITY_SUCCESS = 0.20  # 20% chance flashes land
    POST_PLANT_ANXIETY_THRESHOLD = 15000
    POST_PLANT_ANXIETY_PENALTY = 0.08  # Small attacker penalty under time pressure

    # V5 FIX: Clutch factor
    CLUTCH_FACTOR_BONUS = 0.08  # Small bonus for clutch situations

    # V4: Weapon advantage (NOT capped)
    WEAPON_ADVANTAGE_MULTIPLIER = 0.30
    SHIELD_ADVANTAGE = 0.08
    ...
    async def run_all_scenarios(self, rounds_per_scenario: int = 50):
        print(f"\n{'='*70}")
        print("SCENARIO ANALYSIS V4 - WEAPON ADVANTAGE FIX")
        print(f"{'='*70}")
        print("Key changes from V3:")
        print("  - WEAPON ADVANTAGE: Separate from numbers, NOT capped (up to ±30%)")
        print("  - SHIELD ADVANTAGE: 8% bonus for having armor")
        print("  - RETAKE UTILITY: Increased to 50% success rate")
        print("  - DEFUSE PRESSURE: Increased to 15% bonus")
        print("  - POST-PLANT ANXIETY: Increased to 12% penalty")
    ...
    def _get_config(self, scenario: Scenario, map_data: Dict) -> Dict:
        config = {
            ...
            'attacker_shields': [True] * 5,  # NEW
            'defender_shields': [True] * 5,  # NEW
            'spike_planted': False,
            'spike_site': None,
            'start_time_ms': 0,
            'is_retake': False,
        ...
        if scenario == Scenario.ATTACK_EXECUTE_FAST:
            ...
            config['is_retake'] = True  # Defenders must push to defuse
            ...
            # Attackers eco (classic, no shield) vs Defenders full (vandal, shield)
            config['attacker_weapons'] = ['classic'] * 5
            config['attacker_shields'] = [False] * 5

        elif scenario == Scenario.FULL_VS_ECO:
            # Attackers full vs Defenders eco
            config['defender_weapons'] = ['classic'] * 5
            config['defender_shields'] = [False] * 5
    ...
    def _setup_players(self, engine, map_data, config):
        ...
        for i in range(config['num_attackers']):
            ...
            player.shield = 50 if config['attacker_shields'][i % len(config['attacker_shields'])] else 0
        ...
        for i in range(config['num_defenders']):
            ...
            player.weapon = WeaponDatabase.get_weapon(weapon_name)
            player.shield = 50 if config['defender_shields'][i % len(config['defender_shields'])] else 0
            engine.players[player_id] = player
    ...
    async def _run_fixed_loop(self, engine, map_data, scenario, config) -> Dict:
        ...
        while time_ms < max_time:
            ...
            # Retake utility (defenders flash)
            ...
            for attacker in list(engine.players.values()):
                ...
                for defender in list(engine.players.values()):
                    ...
                    # Build combat context with weapon info
    ...
    def _build_combat_context(self, attacker, defender, engine, map_data, config,
        ...
        # Get weapon tiers from weapon object
        ...
        attacker_weapon_tier = WEAPON_TIERS.get(attacker_weapon, 0.9)
        defender_weapon_tier = WEAPON_TIERS.get(defender_weapon, 0.9)
        ...
        return CombatContext(
            ...
            attacker_weapon_tier=attacker_weapon_tier,
            defender_weapon_tier=defender_weapon_tier,
            attacker_has_shield=(attacker.shield > 0),
            defender_has_shield=(defender.shield > 0),
        )

    def _calculate_advantage(self, ctx: CombatContext, config: Dict) -> float:
        """Calculate attacker advantage with SEPARATED weapon and numbers modifiers."""

        # Base advantage (slight defender edge)
        adv = 0.47

        # ===========================================
        # V4 FIX: WEAPON ADVANTAGE (NOT CAPPED)
        # ===========================================
        weapon_diff = ctx.attacker_weapon_tier - ctx.defender_weapon_tier
        weapon_advantage = weapon_diff * self.WEAPON_ADVANTAGE_MULTIPLIER
        adv += weapon_advantage

        # ===========================================
        # V4 FIX: SHIELD ADVANTAGE (NOT CAPPED)
        # ===========================================
        if ctx.attacker_has_shield and not ctx.defender_has_shield:
            adv += self.SHIELD_ADVANTAGE
        elif ctx.defender_has_shield and not ctx.attacker_has_shield:
            adv -= self.SHIELD_ADVANTAGE

        # ===========================================
        # FLASH PENALTY (from V3)
        # ===========================================
        ...
        # ===========================================
        # NUMBERS ADVANTAGE (CAPPED at ±10%)
        # ===========================================
        ...
        # ===========================================
        # POST-PLANT ANXIETY (from V3, increased)
        # ===========================================
        ...
        # V5 FIX: CLUTCH FACTOR
        # ===========================================
        # Attackers get bonus when in clutch situations (at numbers disadvantage)
        if config.get('is_clutch') and ctx.attacker_team_alive < ctx.defender_team_alive:
            adv += self.CLUTCH_FACTOR_BONUS
    ...
    def print_results(self):
        print(f"\n{'='*70}")
        print("V4 RESULTS - WEAPON ADVANTAGE FIX")
        print(f"{'='*70}")
...
async def main():
    analyzer = FixedScenarioAnalyzerV4()
    ...
    output_file = Path(__file__).parent / 'scenario_analysis_v4_results.json'
    ...
    with open(output_file, 'w') as f:
        json.dump({
            ...
            'version': 'v4',
            'key_changes': [
                'weapon_advantage_not_capped',
                'shield_advantage_added',
                'retake_utility_increased',
                'defuse_pressure_increased',
                'post_plant_anxiety_increased',
            ],
